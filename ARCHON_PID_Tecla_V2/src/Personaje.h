...
public:
	...
	int 	 GetRangoMax()     const { return Player.rango_max; }      // Rango maximo del ataque (en unidades del juego, para calcular alcance de proyectiles)
    ...
    ~Personaje() {
        for (int i = 0; i < (int)Frames_MOV.size(); i++) {
            if (i < (int)shadow_MOV.size())         UnloadModel(shadow_MOV[i]);
            if (i < (int)Frames_MOV_shadow.size())  UnloadTexture(Frames_MOV_shadow[i]);
            UnloadTexture(Frames_MOV[i]);
        }
        for (int i = 0; i < (int)Frames_STILL.size(); i++) {
            if (i < (int)shadow_STILL.size())        UnloadModel(shadow_STILL[i]);
            if (i < (int)Frames_STILL_shadow.size()) UnloadTexture(Frames_STILL_shadow[i]);
            UnloadTexture(Frames_STILL[i]);
        }
        for (int i = 0; i < (int)Frames_ATK.size(); i++) {
            if (i < (int)shadow_ATK.size())          UnloadModel(shadow_ATK[i]);
            if (i < (int)Frames_ATK_shadow.size())   UnloadTexture(Frames_ATK_shadow[i]);
