...
namespace Archon_PID {
    ...
    void App::Init() {
        ...
        gs.controlesOpciones[2].valor = 1;
        ToggleFullscreen();
        Particles::init(gs);
        ...
        screens[(int)TABLERO] = std::make_unique<TableroScreen>();
    }
    ...
    void App::FinalizarTransicion() {
        ...
        if (gs.estadoActual != gs.siguienteEstado) {
            ...
            // Pausar musica global al entrar en combate o tablero, reanudar al salir
            if (gs.estadoActual == CUCHAU_COMBATE || gs.estadoActual == TABLERO) PausarMusicaGlobal();
            bool saleDeCombateOTablero = (anterior == CUCHAU_COMBATE || anterior == TABLERO);
            bool entraCombateOTablero = (gs.estadoActual == CUCHAU_COMBATE || gs.estadoActual == TABLERO);
            if (saleDeCombateOTablero && !entraCombateOTablero) ReanudarMusicaGlobal();
    ...
    void App::Update() {
        ...
        if (gs.slashActivo) {
            ...
            if (gs.slashX >= 0 && gs.estadoActual != gs.siguienteEstado) {
                ...
                // Pausar musica global al entrar en combate o tablero, reanudar al salir
                if (gs.estadoActual == CUCHAU_COMBATE || gs.estadoActual == TABLERO) PausarMusicaGlobal();
                bool saleDeCombateOTablero2 = (anterior == CUCHAU_COMBATE || anterior == TABLERO);
                bool entraCombateOTablero2 = (gs.estadoActual == CUCHAU_COMBATE || gs.estadoActual == TABLERO);
                if (saleDeCombateOTablero2 && !entraCombateOTablero2) ReanudarMusicaGlobal();
                if (auto* scr = GetCurrentScreen()) scr->OnEnter(gs);
            }
    ...
    void App::Draw() {
        ...
        // Cuchau and Tablero set their own projection — skip ARCHON's 800x600 ortho
        if (gs.estadoActual != CUCHAU_COMBATE && gs.estadoActual != TABLERO) {
        ...
        if ((gs.estadoActual == CUCHAU_COMBATE || gs.estadoActual == TABLERO) && gs.slashActivo) {
    ...
    void App::HandleInput() {
        ...
        if ((IsKeyPressed(KEY_P) || IsKeyPressed(KEY_ESCAPE)) &&
            (gs.estadoActual == JUGAR_PVP || gs.estadoActual == JUGAR_IA || gs.estadoActual == TABLERO)) {
