...
namespace Archon_PID {
    ...
    enum Estado {
        ...
        TABLERO,               // Juego Completo: tablero + combate integrado
    };

    enum ModoPartida { MODO_NINGUNO, MODO_COMBATE, MODO_COMPLETO };
    ...
    class ControladorCombate;   // forward declaration for combateActivo
    class Controlador_Tablero;  // forward declaration for tableroActivo
    ...
    struct GameState {
        ...
        ModoPartida modoActual = MODO_NINGUNO;
        int       opcionModoSel = 0;
        ...
        // Puntero al combate activo (para guardar/cargar desde la pausa global)
        ControladorCombate* combateActivo = nullptr;
        bool combateModoIA = false;
        int  combateDificultad = 1;

        // Puntero al tablero activo (para guardar desde la pausa global)
        Controlador_Tablero* tableroActivo = nullptr;
        int partidaActualIdx = -1;  // índice en gs.partidas de la partida en curso
        std::string boardStatePendiente;  // estado serializado pendiente de cargar
        ...
        // Enciclopedia
        int paginaLibro = 0;
        static const int TOTAL_PAGINAS = 17;
