...
namespace Archon_PID {
    ...
    void Screens::enciclopedia(GameState& gs) {
        ...
        if (gs.paginaLibro == 16) {
            ...
            Drawing::texto12(220 + ox, 92,
                std::to_string(gs.paginaLibro * 2 + 1),
                CFloat(0.22f, 0.12f, 0.02f));
            ...
            Drawing::circuloLinea(cx, cy, 55, 36, CFloat(0.5f, 0.35f, 0.07f));
            Drawing::circuloLinea(cx, cy, 42, 36, CFloat(0.5f, 0.35f, 0.07f));
            rlBegin(RL_LINES);
            ...
            for (int k = 0; k < 2; k++) {
                for (int i = 0; i < 6; i++) {
                    ...
                    rlVertex2f(cx + cosf(a1) * 30, cy + sinf(a1) * 30);
                    rlVertex2f(cx + cosf(a2) * 30, cy + sinf(a2) * 30);
                }
            }
            rlEnd();

            // Número de página derecha
            Drawing::texto12(555 + ox, 92,
                std::to_string(gs.paginaLibro * 2 + 2),
                CFloat(0.22f, 0.12f, 0.02f));
            ...
            // Nombre de la tropa con sombra sepia en la página derecha
            Drawing::texto24(432 + ox, 503, tropa.nombre, CFloat(0.55f, 0.35f, 0.08f));  // sombra
            Drawing::texto24(430 + ox, 505, tropa.nombre, CFloat(0.05f, 0.02f, 0.01f));  // tinta
            // Descripción de la tropa
            ...
            // Emblema de facción: estrella de seis puntas (Luz) o pentagrama (Oscuridad)
        ...
        // Flechas de cambio de página, pulsantes en el tiempo
        ...
        // Título inferior con efecto de pulso suave
